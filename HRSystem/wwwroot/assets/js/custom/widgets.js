...
var KTWidgets = function() {
	...
	var initMixedWidget2 = function () {
		...
		$.ajax({
			url: "/Admin/Home/GetNewReg",
			type: 'GET',
			dataType: 'json', // added data type
			success: function (res) {
				$(".newReg").text(res[0].count)
				var element = document.getElementById("GetNewReg");
				...
				var options = {
					series: [{
						name: 'Count',
						data: [res[0].count, res[1].count, res[2].count, res[3].count, res[4].count, res[5].count, res[6].count]
					}],
					chart: {
						fontFamily: 'inherit',
						height: height,
					...
					xaxis: {
						...
						categories: [res[0].day, res[1].day, res[2].day, res[3].day, res[4].day, res[5].day, res[6].day],
						position: 'top',
						axisBorder: {
							show: false
						},
	...
	var initStatisticsWidget1 = function () {
		...
		$.ajax({
			url: "/Admin/Home/GetViews",
			type: 'GET',
			dataType: 'json', // added data type
			success: function (res) {
				data = res;
				$(".ViewsNum").text(res[0].views)

				var charts = document.querySelectorAll('.GetViews');
